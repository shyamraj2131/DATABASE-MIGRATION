...
The first check counts the rows in all five tables and compares them with the pre-migration counts. The second check looks for orphaned appointments — appointments that have no matching patient or doctor. The third check looks for orphaned medical records — records with no matching appointment. The fourth check looks for NULL values in important columns like patient name and doctor name. The fifth check confirms there are no negative salary values. The sixth check confirms there are no negative bill amounts. The seventh check adds up all bill amounts and confirms the total matches the source database exactly at 9800 rupees. The eighth check picks the first three patients and compares them with the source to confirm the data is identical.
